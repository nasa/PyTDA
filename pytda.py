...
Major References
----------------
Bohne, A. R. (1982). Radar detection of turbulence in precipitation environments. 
    Journal of the Atmospheric Sciences, 39(8), 1819-1837.
...
Labitt, M. (1981). Coordinated radar and aircraft observations of turbulence
    ...
    Development Service.
Williams, J. K., L. B. Cornman, J. Yee, S. G. Carson, G. Blackburn, and J. Craig, 
    2006: NEXRAD detection of hazardous turbulence. 44th AIAA Aerospace Sciences
...
NEXRAD data (V06 and earlier), recommend running on UFs produced from the native
Level 2 files via Radx due to conflicts between PyART and older NEXRAD data
models.
...
1. Fixed a bug that prevented actual filtering of DZ/SW fields before turbulence
   calculations in certain radar volumes. Performance improved considerably!
...
def calc_turb_sweep(radar, sweep_number, radius=DEFAULT_RADIUS, split_cut=False,
    ...
    """
    Provide a Py-ART radar object containing 'reflectivity' and 'spectrum_width'
    ...
    #Radar location needed to get lat/lon of every gate for distance calculations
    ...
    for j in xrange(sweep_size):
        for i in xrange(radar.ngates):
            sw_lonr[j,i] = klonr +\
                atan2c_longitude(sw_azr_1d[j], sw_gr[j,i], klatr, sw_latr[j,i])
    ...
    #Restore turbulence to a full 2-D sweep array and return along with lat/lon.
    turb_radar = sw_lat.flatten() * 0.0 + fill_val_sw
    turb_radar[condition] = turb_radar_f
...
def calc_turb_vol(radar, radius=DEFAULT_RADIUS, split_cut=False,
                  ...
                  gate_spacing=DEFAULT_GATE_SPACING):
    """
    Leverages calc_turb_sweep() to process an entire radar volume for turbulence.
    Has ability to account for split-cut sweeps in a volume (i.e., DZ & SW on
    different, mismatched sweeps).
    radar = Py-ART radar object
